...
function MessageBubble({ message }) {
  ...
  if (isUser) {
    return (
      <div className="flex justify-end">
        <div className="max-w-[82%] rounded-2xl rounded-br-md bg-brand-primary px-4 py-3 text-[13px] leading-6 text-white shadow-sm lg:max-w-[68%]">
          {message.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="max-w-[84%] rounded-2xl rounded-bl-md border border-brand-primary/10 bg-brand-light px-4 py-3 text-[13px] leading-6 text-brand-dark shadow-sm lg:max-w-[70%]">
        {message.content}
      </div>
    </div>
  );
...
export default function Chat({
  ...
  return (
    <div className="flex h-full min-h-[calc(100vh-9.5rem)] flex-col lg:min-h-[calc(100vh-8rem)]">
      <div className="rounded-lg border border-brand-primary/10 bg-[var(--bg-muted)] px-4 py-3 text-sm leading-6 text-[var(--text-main)] shadow-sm lg:flex lg:items-center lg:justify-between lg:gap-5">
        <div className="text-xs font-semibold uppercase text-brand-dark">
          Meet Priya
        </div>
        <p className="mt-2 mb-0 max-w-2xl text-[13px] text-[var(--text-muted)] lg:mt-0">
      ...
      <div className="mt-4 space-y-3 border-t border-[var(--border-soft)] pt-4">
        <div className="scrollbar-hidden flex gap-2 overflow-x-auto pb-1 lg:flex-wrap lg:overflow-visible">
        ...
        <form onSubmit={submitMessage} className="flex items-end gap-3">
          ...
          <textarea
            ...
            className="max-h-28 min-h-[48px] flex-1 resize-none rounded-lg border border-[var(--border-soft)] bg-[var(--bg-panel)] px-4 py-3 text-[13px] leading-6 text-[var(--text-main)] shadow-sm transition focus:outline-none focus:ring-2 focus:ring-brand-primary/40"
          />
          <button
            type="submit"
            className="rounded-lg bg-brand-primary px-4 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-primary/40"
          >
            {lang === 'hi' ? 'भेजें' : 'Send'}
          </button>
        </form>
